  ...
  if (j < grid[i].length - 1 && grid[i][j + 1] === plot) {
    ...
    if (
      !(
        i > 0 &&
        grid[i - 1][j] === plot &&
        (j === grid[0].length - 1 || grid[i - 1][j + 1] !== plot)
      )
    ) {
      regionData.sides += 1;
    }
  ...
  if (i < grid.length - 1 && grid[i + 1][j] === plot) {
    ...
    if (
      !(
        j < grid[0].length - 1 &&
        grid[i][j + 1] === plot &&
        (i === grid.length - 1 || grid[i + 1][j + 1] !== plot)
      )
    ) {
      regionData.sides += 1;
    }
